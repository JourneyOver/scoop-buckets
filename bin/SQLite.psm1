...
$SQLiteLoaded = $false
# Download the SQLite NuGet package if it does not exist
...
function New-SQLiteConnection {
  ...
  #>
    param (
        ...
        [Parameter(HelpMessage = 'Return the connection object instead of discarding it')]
        [switch]
        $Passthru
    )
...
function Invoke-SQLiteQuery {
    ...
    # Store each row of the result set in an object
    $results = @()
    While ($SQLiteReader.Read()) {
        # Add the current row's data to the array
        $row = @{}
